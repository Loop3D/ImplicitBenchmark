...
We now propose a review of the main methods proposed to compute three-dimensional scalar fields in implicit geological structural modeling. 
...
\item \textbf{The visualization / evaluation}. DcK and RBF approaches are meshless approaches, in the sense that the solution is defined everywhere in space. Once the system has been solved, the evaluation for a given position is given directly by Eq. \ref{eq:basis}. It only involves to compute the distance between the evaluation point and the data points. For visualization and further processing, the solution is generally evaluated at the vertices of a Cartesian grid or unstructured mesh. Conversely, DSI-based methods directly run the computation on a discrete support. Changing the support implies running the computation again. In both cases, the visualization of geological surfaces can be achieved using polygonal isosurface extraction algorithms such as the marching cubes \citep{Calcagno2008PEPI} or marching terahedra method \citep{Frank2007CG}. \todo[inline]{Did anyone try ray tracing with meshless approaches ?}
...
\begin{table}
\centerline{\begin{tabular}{|p{15mm}|p{15mm}|p{15mm}|p{20mm}|p{3cm}|p{20mm}|l|p{3cm}|}
\hline
Name & Method & Computa\-tion & Visualiz\-ation & Reference(s) & Main language / dependency & License & Main parameters \\
\hline
GemPy & DcK & Meshless & Marching cubes & \citet{delaVarga2018GMDD} & Python / Theano & LGPL & Type and parameters of variogram model \\
\hline
LoopStru\-ctural & DSI & Cartesian & Marching cubes & In prep. & Python & MIT & Least squares weights for data / regularization \\
\hline 
...
\hline 
SIGMA & DSI & Cartesian & Marching cubes & \citet{Irakarama2018EAGE} & C++/CUDA & RING & Least squares weights for data / regularization \\
\hline
StructuralLab & DSI & Tet.
 mesh & Marching tetra & \citet{Frank2007CG,Caumon2013GaRSITo} & C++ / SKUA-GOCAD & RING & Least squares weights for data / regularization \\
\hline
SurfE & DcK & Cartesian & Marching cubes & \citet{Hillier2014MG} & C++ & MIT & Type and coefficients of RBF functions; interpolation or approximation \\ 
\hline
\end{tabular}}
...
As in the Hecho data set, we did a rough estimate of average layer thicknesses to choose the reference relative geological time values given in Table \ref{tab:Claudius}. 
