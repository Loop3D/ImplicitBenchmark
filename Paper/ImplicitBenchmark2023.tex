...
\todo[inline]{Need to change the end of the abstract}
...
\todo[inline]{Mike: "From this benchmark, what are the critical forms of geological knowledge that need to be incorporated into the structural modelling other than the noted thickness variations? Topological?" }
...
\todo[inline]{
I removed some considerations and defer them to the discussion. Previous comments: 
...
- About the ability to predict the reference surface away from data, as we discussed some time ago, I think this is questionable.
  ...
  might give different results. We can certainly compare and comment on these results but I don't see how we can decide which are "better" since we are in a case of no a priori. " 
	...
	GL: "`I think you are apologising too much... and some of this point should be moved to the discussion rather than giving the reader the feeling that... really? why bother reading this paper if everything you say later is all biased and non-exhaustive and case dependent etc... (I am badly plagiarising...) This is killing the impact of the paper."
	
	MH: "`For the three case studies, are there metrics that are more informative in evaluating a method's modelling performance?"'
...
\todo[inline]{Mike : "`Perhaps a note on how f1, ..., fK are ordered - e.g. f1 > ... > fK. That is how I define the order (the relative scalar values need to be compatible with the orientation data). My younging direction (polarity of normal) specifies the direction of younger stratigraphy which have larger scalar values."' 

GC: Not sure I understand if this ordering is required or just a convenience. }
...
Essentially, these methods make different choices about the way data are handled, the type of basis function used to approximate the solution, and the mathematical formulation to ensure the uniqueness of the solution. These differences have an impact on the way the solution is evaluated and visualized. \todo{Julien: I kept visualized : marching cubes or marching tetra is one example}
...
\todo{JR: Not fully clear. 

GC: I guess the last part of the sentence might need some reformulation, as the shape of the isosurface should change at least close to the points. At the extreme, if the covariance at 0 is 0, then the approach will amount to some sort of classical regression, right ?} 
As in classical kriging, it is also possible to add a nugget effect to the covariance function to transform the exact interpolation into an approximation. In this case, the nugget value relatively to $C_0$ controls the degree of approximation (exact if zero, polynomial if equal to $C_0$. This feature is also used in the SurfE code used in this study \citep{Hillier2014MG}. %. and MSS {\citep{Renaudeau2019MG}. 
...
\noindent where $\varepsilon$ controls the flatness of the basis function as the distance $r$ increases. \todo{GC: Needs clarification. How is epsilon inferred ?}
